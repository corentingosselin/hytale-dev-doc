...
1. **For Commands**: Use `AbstractPlayerCommand` instead of `CommandBase`
   ```java
   // WRONG - CommandBase runs on scheduler thread
...
**Problem**: Players get kicked or can't join due to token issues.

**Solutions**:

1. **Clock Sync Issue**: Player's PC clock may be wrong
   - Have player check and update their system time
   - This has fixed issues for multiple community members
