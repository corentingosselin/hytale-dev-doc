...
See [Java Version Requirements](/getting-started/java-version) for installation instructions.
...
See [Java Version Requirements](/getting-started/java-version) for detailed installation guides.
...
3. Configure JDK 25 in Project Structure > SDKs

**Eclipse:**
1. Download Eclipse IDE for Java Developers
2. Install Buildship Gradle plugin
3. Configure JDK 25 in Installed JREs
...
# Linux/macOS
cp ~/.var/app/com.hypixel.HytaleLauncher/data/Hytale/install/release/package/game/latest/Server/HytaleServer.jar libs/
```

> **Important**: There is no official Maven repository for the Hytale API yet. You must use the local JAR from your server installation as a `compileOnly` dependency.
...
# 5. Start server
cd /path/to/server && java -jar HytaleServer.jar
```

### HotSwap for Development

For faster iteration during development, use external HotSwap tools:

1. **DCEVM/JBR** - Enhanced JVM with better hotswap support
2. **HotSwapAgent** - Plugin for runtime class reloading
...
This allows minor code changes to be applied without full restart (method body changes only).
